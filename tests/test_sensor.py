"""Tests for SunYou sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.sunyou.const import ParcelStatus
from custom_components.sunyou.sensor import (
    SunYouAwaitingPickupSensor,
    SunYouDeliveredParcelsSensor,
    SunYouIncomingParcelsSensor,
    SunYouLastUpdateSensor,
    SunYouNextDeliverySensor,
    SunYouParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = SunYouIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = SunYouParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = SunYouParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = SunYouNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = SunYouNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = SunYouDeliveredParcelsSensor(coordinator, _entry())
    assert sensor.native_value == 1
...
def test_last_update_sensor():
    ...
    sensor = SunYouLastUpdateSensor(coordinator, _entry())
