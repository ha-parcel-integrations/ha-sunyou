"""Tests for the SunYou coordinator: fetching, caching and events.
...
from custom_components.sunyou.api import SunYouApiError
...
from custom_components.sunyou.coordinator import SunYouCoordinator
...
# active_sample() is already IN_TRANSIT (newest event 206
# DestinationAirPortArrival); out_for_delivery_sample() is one leg further
# along (2071 OutForDelivery) — the two together give a genuine status
# transition for the event tests below.
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = SunYouApiError("HTTP 500")
...
async def test_update_backfills_missing_order_no(hass):
    """An edge payload without ``orderNo`` (the barcode field) keeps the
    requested code — never ``trackingNumber``, the last-mile carrier's
    number."""
    entry = _entry_with([{CONF_TRACKING_CODE: OTHER_CODE}])
    entry.add_to_hass(hass)
...
async def test_event_carries_device_id(hass):
    ...
    client.async_get_parcel.return_value = active_sample()
    await coordinator._async_update_data()
    client.async_get_parcel.return_value = out_for_delivery_sample()
    await coordinator._async_update_data()
    await hass.async_block_till_done()
...
async def test_fires_status_changed_event(hass):
    ...
    client.async_get_parcel.return_value = active_sample()  # in transit
...
async def test_delivery_time_changed_never_fires_without_an_eta(hass):
    """SunYou's payload carries no forecast field at all (`transitDays` is
    *elapsed* days on a delivered parcel, not an ETA) — `planned_from` /
    `planned_to` are always None, so this event can never fire. A genuine
    status transition must not accidentally trip it either."""
    entry = _entry_with([{CONF_TRACKING_CODE: ACTIVE_CODE}])
    entry.add_to_hass(hass)
    client = AsyncMock()
    coordinator = SunYouCoordinator(hass, client, entry)
