"""Coordinator for the SunYou parcel tracker integration.
...
from .api import SunYouApiClient, SunYouApiError
from .const import (
...
class SunYouCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (SunYouApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                _LOGGER.warning("SunYou fetch failed for %s: %s", code, result)
            ...
            if result is None:
                # Unknown code, or not scanned yet (SunYou answers HTTP 200,
                # has: false for both). Keep prior data if we have it,
                # otherwise show a pending placeholder so the user still sees
                # the parcel they asked us to track. ``orderNo`` — not
                # ``trackingNumber``, which is the last-mile carrier's number,
                # not SunYou's own barcode.
                raws_by_code[code] = self._raw_cache.get(code) or {"orderNo": code}
                continue

            # The response's own barcode can be missing on edge payloads;
            # fall back to the code we asked for so the sensor keeps its key.
            result.setdefault("orderNo", code)
        ...
        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("SunYou unreachable for all tracked parcels")
