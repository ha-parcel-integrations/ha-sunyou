"""Calendar platform for the SunYou parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the SunYou deliveries calendar from a config entry."""
    async_add_entities([SunYouDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class SunYouDeliveriesCalendar(CoordinatorEntity[SunYouCoordinator], CalendarEntity):
    """A read-only calendar of expected SunYou deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "SunYou parcel")
            description_parts = [
