"""Sensor platform for the SunYou parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: SunYouConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up SunYou sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        SunYouIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        SunYouAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            SunYouParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(SunYouNextDeliverySensor(coordinator, entry))
    entities.append(SunYouDeliveredParcelsSensor(coordinator, entry))
    entities.append(SunYouLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class SunYouIncomingParcelsSensor(
    CoordinatorEntity[SunYouCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                SunYouParcelSensor(self.coordinator, self._entry, barcode)
...
class SunYouAwaitingPickupSensor(
    CoordinatorEntity[SunYouCoordinator], SensorEntity
):
    ...
    def __init__(self, coordinator: SunYouCoordinator, entry: ConfigEntry) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class SunYouParcelSensor(CoordinatorEntity[SunYouCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked SunYou parcel."""
    ...
    def __init__(
        self, coordinator: SunYouCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class SunYouNextDeliverySensor(
    CoordinatorEntity[SunYouCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: SunYouCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class SunYouDeliveredParcelsSensor(
    CoordinatorEntity[SunYouCoordinator], SensorEntity
):
    """Recently delivered tracked SunYou parcels."""
...
class SunYouLastUpdateSensor(
    CoordinatorEntity[SunYouCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when SunYou was last polled successfully."""
