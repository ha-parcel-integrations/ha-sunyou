# Automatic parcel tracking from e-mail (IMAP → SunYou)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the SunYou tracking code, and register it with `sunyou.track_parcel` — fully automatic, no extra custom component required.

SunYou is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `sunyou.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ sunyou.track_parcel
                                     │
...
- This integration, with the `sunyou.track_parcel` action available (field `tracking_code`).
...
```
(?<![0-9A-Za-z])SY[0-9A-Za-z]{6,}
```

Every SunYou number captured in research starts with **`SY…`**, though only
the `SYAE…` variant (e.g. `SYAE006809461`) has actually been seen — the regex
anchors on the shorter, confirmed prefix rather than guessing at the rest.
The integration itself accepts any code matching `^[A-Z0-9]{6,30}$`, but
matching *that* broadly against e-mail text would hit every order number in
the mail — so the example anchors on the known prefix instead. **If your
mails carry a different, more specific prefix, tighten the alternation**
(`(?:SYAE|YOURPREFIX)`); the AI fallback will surface unknown formats in the
meantime.
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\bSY…')` silently becomes a **backspace character** (`\b` is a string escape), so the regex never matches — no error anywhere. That's why the pattern is backslash-free: `(?<![0-9A-Za-z])` lookarounds instead of `\b`, `[0-9]` instead of `\d`. Copy that style if you add a prefix.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@sunyoushipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: SYAE006809461","initial":true,"folder":"INBOX","username":"test"}'
```

Then `sunyou.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
